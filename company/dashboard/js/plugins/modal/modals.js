		...
		$.ajax({
			...
			success	: function(ver){
				...
				var modal = {
					...
					body		:	{
						content		:	'<textarea class="ver-link-output" spellcheck="false" readonly="readonly">www.theroishop.com/calc-your-roi/2/?roi=' + getUrlVars()['roi'] + '&v=' + ver + '</textarea>'
					},
					footer		:	{
		...
		$.ajax({
			...
			success	: function(contributors){
				...
				$.each(currentContributors, function(index, value) {
					...
					modalBody += 	'<tr data-contributor-id="' + value.id + '">\
										<td class="project-title">' + value.username + '</td>';
	...
	$(document).ready(function() {
		...
		$('.modal').on('click', '.remove-contributor', function() {
			...
			$.ajax({
				type	: 	"GET",
				url		:	"ajax/calculator.get.php",
				data	:	'action=delcont&id=' + contributorId,
				success	: function(){
		...
		$('.calculator-popup').on('click', function() {
			...
			$('#wrapper').calx('getCell', cellIdentifier).setFormula( originalFormula );
			
			// Get all the dependencies of the calculation clicked
			var cellDependencies = $('#wrapper').calx('getSheet').getCell(cellIdentifier).buildDependency();
			...
			var panel = $('#wrapper').calx('getSheet').getCell(cellIdentifier).buildInputPanels();

			var cellFormula = $('#wrapper').calx('getSheet').getCell(cellIdentifier).getFormula();
			
			var cellFormat = $('#wrapper').calx('getSheet').getCell(cellIdentifier).getFormat();
			
			for(a in $('#wrapper').calx('getSheet').getCell(cellIdentifier).dependencies) {
			...
			$('#wrapper').calx('getCell', cellIdentifier).setFormula( currentFormula );
		...
		$('.modal').on('click', '.override-output-confirm', function() {
			...
			$('#wrapper').calx('getCell', cellIdentifier).setFormula( overriddenValue + ' * 1' );
		...
		$('.modal').on('click', '.original-equation', function() {
			...
			$('#wrapper').calx('getCell', cellIdentifier).setFormula( originalEquation );
			$('#wrapper').calx('getSheet').calculate();
