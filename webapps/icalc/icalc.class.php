...
class icalc
{
	...
	public function get_roi_tables($wbroiID){
		$sql = "SELECT * 
    		FROM `wb_roi_tables` t1
    		WHERE wbroiID=:wbroiID  AND LongTable=0;";
	...
	public function get_roi_table_data($wbroiID,$tableID){
		$sql = "SELECT * 
    		FROM `wb_roi_table_values` t1
    		WHERE wbroiID=:wbroiID AND tableID=:tableID
			ORDER BY row, col;";
	...
	public function formattablecell($cellarray,$tablepre){
		//This function will parse a cell array data into a viewable element
		$returnhtml = '';
		$returnhtml = '';
		$returnhtml = $returnhtml + '<input'; 
        $returnhtml = $returnhtml + 'tabindex=200';
        $returnhtml = $returnhtml + 'id="' . $tablepre . $cellarray['col'] . $cellarray['row'] . '"';
        $returnhtml = $returnhtml + 'data-cell="' . $cellarray['col'] . $cellarray['row'] . '"'; 
        $returnhtml = $returnhtml + 'type="text" ';
        $returnhtml = $returnhtml + 'class="form-control roishop-wb-field b-r-sm"'; 
        $returnhtml = $returnhtml + 'data-format="' . $cellarray['format'] . '"'; 
	...
	//Function that adds an IP address to the wb_roi_instance table
	public function update_stdvalues($wbroiID,$instanceID,$ip,$country,$state,$city,$lat,$long){

		$sql = "INSERT INTO wb_roi_instance_values_standard
				(`wb_roi_ID`,`instanceID`,`stdfieldID`,`value`)
				VALUES
				(:wbroiID,:instanceID,1,:ip)";
   ...
   return $ipaddress;
	...
	public function getMobileRedirect($reportID){
		
		$sql = "SELECT t1.MobileRedirect
				FROM wb_roi_reports t1
				WHERE t1.wb_roi_report_ID = :reportID
				LIMIT 1;";
				
		$stmt = $this->_db->prepare($sql);
		$stmt->bindParam(':reportID', $reportID, PDO::PARAM_INT);
		$stmt->execute();
		$rpt = $stmt->fetchColumn();
		return $rpt;
