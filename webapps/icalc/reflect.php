...
<?php 
	...
	//A-1. Get the instanceID from the url to pull values for this form/instance.
	if(isset($_GET['i'])) {
    $instanceID 	= $_GET['i'];
	}else{$instanceID=0;}
	...
	//1.a - Replace tags of fields with the values already given for this instance.
	
	//Need to loop through the standard values table and replace accodingly
		 $SQL = "SELECT * 
	    		FROM `wb_roi_reports_fieldusage` t1
	    		JOIN `wb_roi_instance_values` t2 
	    		ON t1.fieldID=t2.field
		...
		if($numrows>0){
		  foreach($list as $r){
			$x = $x + 1;	
			$roihtml = str_replace('<tagrpt>' . $r['usageID'] . '</tagrpt>', $formatter->format($r['value'],$r['numeralFormat']) , $roihtml);
		  }
		}
