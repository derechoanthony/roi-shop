...
<?php 
	...
	//0. Retrieve properties about this ROI
	if($reportID==0){
	...
	//Retrieve Table Data to Embed in Report
	
	$tablelist		= $icalc->get_roi_tables($wbappID);
	$tablestring 	= '';
	$alltablestring	= '';
	...
	{
		$starttabletime = microtime(true);
		$tableID 		= $t['tableID'];
		$tablepre 		= $t['tableName'];
		$allcells		= $icalc->get_roi_table_data($wbappID,$tableID);
		$allcelldata = json_encode($allcells);
		$currow = 0;
		
		$tablestring = '<h2 class="lookupheader">Lookup Table: ' . $t['tableName'] . '<tagloaded>' . $tableID . '</tagloaded></h2>';
		
		$tablestring = $tablestring . '<table class="lookuptable1">';
		
		foreach ($allcells as $cell)
		{
			$cellvalue = $cell['cellvalue'];
			//$celldata = json_encode($cell);
			//$tablestring = $icalc->formattablecell($cell, $tablepre);
			if (($currow != $cell['row']) && $currrow > 0) {$startrow = '</tr><tr>';} else {$startrow='';}
			if (($currow != $cell['row']) && $currrow == 0) {$startrow = '<tr>';} else {$startrow='';}
			
			$currow = $cell['row'];
			
			$startcell = ($currow == 0 ? '<th>' : '<td>' );
			$endcell = ($currow == 0 ? '</th>' : '</td>' );
			
			$tablestring = $tablestring . $startrow . $startcell . '<input'; 
			$tablestring = $tablestring . ' tabindex=200';
			$tablestring = $tablestring . ' id="' . $tablepre . $cell['col'] . $cell['row'] . '"';
			$tablestring = $tablestring . ' data-cell="' . $tablepre . $cell['col'] . $cell['row'] . '"'; 
			$tablestring = $tablestring . ' type="text" ';
			$tablestring = $tablestring . ' class="form-control roishop-wb-field b-r-sm"'; 
			$tablestring = $tablestring . ' style="margin: 0px;"'; 
			$tablestring = $tablestring . ' data-format="' . $cell['format'] . '"'; 
			$tablestring = $tablestring . ' value="' . $cell['cellvalue'] . '"';
			$tablestring = $tablestring . ' >' . $endcell;
		}

			$tablestring = $tablestring . '</tr></table>';

		//Look Through report and replace if it is called for
		
		$endtabletime = microtime(true);
		$totaltabletime = number_format(($endtabletime - $starttabletime), 2);
		
		
		$roihtml = str_replace("<tagtable>" . $tableID . "</tagtable>" , $tablestring , $roihtml);
		$roihtml = str_replace("<tagloaded>" . $tableID . "</tagloaded>" , '<h3>Load Time: ' . $totaltabletime . '</h3>' , $roihtml);
	...
	$roihtml = '<body class="calculator-body">
				...
				<img class="poweredlogo" src="https://www.theroishop.com/webapps/assets/img/powered1.png" width="200px" style="text-align:right; padding:0px; float: right; padding-right: 10px;"></img>
                ' . $roihtml . $allcelldata .  '
				</form></div>
				' . $roiPreScripts . '
				</body></html>';
