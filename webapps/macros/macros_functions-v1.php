...
function CloseModal(){
	$this->response['nextaction']=4;
	$this->response['modalid'] = 'macromodal';
	return json_encode($this->response);
}
...
function PushLeadToMarketo($instanceID,$mkConnectionID){
	...
	$this->response['mrktosuccess'] = 'false';
	$this->response['mrktofailuremsg'] = 'One or more required fields was empty.';	
...
function sendEmail($recipient,$emailsubject,$emailfrom,$body,$attachment=NULL,$attachmentname=NULL,$replyto=NULL){
	  ...
	  try{
		  ...
		  ->setFrom(array($emailfrom . '@theroishop.com' => $emailfrom))
		  ->setTo(array($recipient))
		  ->setBody($body, 'text/html');

		  if ($attachment && strlen($attachment)>1) {$message->attach(Swift_Attachment::fromPath($attachment)->setFilename($attachmentname));}
		  if ($replyto && strlen($replyto)>1) {$message->setReplyTo(array($replyto => $emailfrom));} 
	

		  if(!$mailer->send( $message, $failures )) {
			  $emailrtn['failures'] = $failures;
		  }	 
		  else 
		  {
			  $emailrtn['failures'] = 'none';
			  $emailrtn['success'] = 'success';
			  $emailrtn['sentTo'] = $recipient;
			  $emailrtn['sentFrom'] = $emailfrom . '@theroishop.com';
			  $emailrtn['Subject'] = $emailsubject;
			  $emailrtn['attachment'] = $attachment;
			  $emailrtn['attachmentName'] = $attachmentname;
			  $emailrtn['replyTo'] = $replyto;
			  //$emailrtn['messageBody'] = $body;
...
function FinishROI($macroID, $instanceID, array $aurgs) {
	...
	if($immedstop>0){
		...
		$savedfile = $g->StorePDFNameValue($instanceID,'https://theroishop.com/webapps/assets/customwb/' . $wbroiID . '/pdf/' . $wbroiID . '-' . $instanceID . '.pdf');
		...
		$fileattach = '../assets/customwb/' . $wbroiID . '/pdf/' . $wbroiID . '-' . $instanceID . '.pdf';
		$fileattachname = $filename . '.pdf';
		...
		if($aurgs['14']==1){
		  ...
		  $this->response['email1'] = $this->sendEmail($client_email,$emailsubject,'ExpressROI',$client_emailbody,$fileattach);
		}
		...
		if($aurgs['13']==1){
		  ...
		  $this->response['email2'] = $this->sendEmail($customer_email,$emailsubject,$emailfrom,$cust_emailbody,$fileattach,$fileattachname,$replyto);
		  ...
		  switch ($aurgs['8']) {
			...
			case 3:
				//Open a modal
				$this->reponse['modalreport'] = $aurgs['9'];
