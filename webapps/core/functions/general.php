...
function analytics_getalldata($roiid) {
	
	$returndata = array();
	
	$sql = "Select * from wb_roi_instance where wbroiID=:roiid";
				
	$stmt = $this->_db->prepare($sql);	
	$stmt->bindParam(':roiid',$roiid, PDO::PARAM_INT);
	$stmt->execute();
	$instances = $stmt->fetchall();
	
	$returndata['instances'] = $instances;
	
	$sql = "Select * from wb_roi_instance_values where wb_roi_ID=:roiid";
				
	$stmt = $this->_db->prepare($sql);	
	$stmt->bindParam(':roiid',$roiid, PDO::PARAM_INT);
	$stmt->execute();
	$instancevalues = $stmt->fetchall();

	$returndata['instancevalues'] = $instancevalues;

	$sql = "SELECT * FROM `wb_roi_instance_values_standard` WHERE `wb_roi_ID`=:roiid";
				
	$stmt = $this->_db->prepare($sql);	
	$stmt->bindParam(':roiid',$roiid, PDO::PARAM_INT);
	$stmt->execute();
	$stdvalues = $stmt->fetchall();
	
	$returndata['stdvalues'] = $stdvalues;

	return $returndata; 
}

function analytics_getalldata1($roiid) {
	...
	$sql = "Select * from wb_dashboard_table_columns where wbroiID=:roiid";
				
	$stmt = $this->_db->prepare($sql);	
	$stmt->bindParam(':roiid',$roiid, PDO::PARAM_INT);
	$stmt->execute();
	$tablecols = $stmt->fetchall();

	$returndata['tablecols'] = $tablecols;
	
	
	$sql = "Select * from wb_roi_fields where wb_roi_ID=:roiid";
				
	$stmt = $this->_db->prepare($sql);	
	$stmt->bindParam(':roiid',$roiid, PDO::PARAM_INT);
	$stmt->execute();
	$calcfields = $stmt->fetchall();

	$returndata['calcfields'] = $calcfields;
	
	return $returndata; 
}

function analytics_getviews($roiid) {
...
function analytics_getvalues($roiid) {
	
	$sql = "Select instanceID, field, value, formatted_value FROM wb_roi_instance_values WHERE instanceID>0 AND wb_roi_ID=:roiid";
...
function analytics_getstdvalues($roiid) {
	
	$sql = "SELECT instanceID, stdfieldID, value FROM wb_roi_instance_values_standard WHERE  instanceID>0 AND wb_roi_ID=:roiid";
...
function analytics_gettablecols($roiid) {
...
function analytics_getcalcfields($roiid) {
