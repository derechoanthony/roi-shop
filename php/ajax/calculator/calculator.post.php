<?php
	...
	if( $_GET['action'] == 'getroiarray' ) {
		...
		if( $roiStoredArray ){
			...
			$roiStructure['elements'] = json_decode($storedArray);
		} else {
			$roiStructure['elements'] = $roiBuilder->getRoiArray();
		};
				...
				if($seriesOptions){
					...
					foreach($seriesOptions as $series){
						...
						foreach($equations as $equation){
							$elementArray['highchart']['series'][$seriesCount]['data']['formula'][] = $seriesEquations[$equation]['equation'];
						};
						$seriesCount++;
					};				
				};
