<?php
	...
	if( ( isset($_GET['v']) || isset($_GET['amp;v']) ) && isset($_GET['roi']) ) {
		...
		$is_verified = $roi_information->check_verification();
	...
	if( isset( $_SESSION['UserId'] ) && isset( $_GET['roi'] ) ) {
		
		// Return the owner of the ROI
			
		$is_owner = $roi_information->verify_owner();
		$has_privilege = $roi_information->check_privilege();
		if( $is_owner || $has_privilege ){
		...
		case 1:
			...
			if( isset($_GET['email']) && $_GET['email']=='false' ){
				...
				if( $_SESSION['roi'] != $_GET['roi'] ){
					...
					$roi_information->verification_link_used();
				}
				break;
			}
