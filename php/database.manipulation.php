<?php
	...
	if( isset($_POST['action']) ) {	
		...
		if( $_POST['action'] == 'addroi' ) {		
			...
			$sql = "INSERT INTO roi_companies (company_id, company_name)
					VALUES(:cid, :cname);";

			$stmt = $db->prepare($sql);
			$stmt->bindParam(':cid', $lastId, PDO::PARAM_INT);
			$stmt->bindParam(':cname', $_POST['name'], PDO::PARAM_STR);
			$stmt->execute();	
			
			$sql = "INSERT INTO roi_company_structures (structure_id, structure_title, company_id)
					VALUES(:sid, :stitle, :cid);";

			$stmt = $db->prepare($sql);
			$stmt->bindParam(':sid', $lastId, PDO::PARAM_INT);
			$stmt->bindParam(':stitle', $_POST['name'], PDO::PARAM_STR);
			$stmt->bindParam(':cid', $lastId, PDO::PARAM_INT);
			$stmt->execute();

			$sql = "INSERT INTO roi_structure_versions (version_id, version_name, structure_id)
					VALUES(:vid, :vname, :sid);";

			$stmt = $db->prepare($sql);
			$stmt->bindParam(':vid', $lastId, PDO::PARAM_INT);
			$stmt->bindParam(':vname', $_POST['name'], PDO::PARAM_STR);
			$stmt->bindParam(':sid', $lastId, PDO::PARAM_INT);
			$stmt->execute();
	...
	if( isset($_GET['action']) ) {
		...
		if( $_GET['action'] == 'resetver' ) {		
			...
			$sql = "UPDATE ep_created_rois SET verification_code = :ver
					WHERE roi_id=:roi";
