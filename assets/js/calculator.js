...
$(document).ready(function() {
	if( sessionStorage.getItem('ajaxcall') )
	{
		if( sessionStorage.getItem('currentroi') === getUrlVars()['roi'] )
		{
			loadURL(sessionStorage.getItem('ajaxcall'), $('#content'));
		} else {
			loadURL('ajax/sections.php?id=dash&roi=' + getUrlVars()['roi'] + ( getUrlVars()['v'] ? '&v=' + getUrlVars()['v'] : '' ), $('#content'));
		}
	} else {
		loadURL('ajax/sections.php?id=dash&roi=' + getUrlVars()['roi'], $('#content'));
	}
	/*
	 * Fire tooltips
	 */
	...
	$('li#discovery ul li a').click(function(e) {
		e.preventDefault();
		
		url = "ajax/sections.php?id="+$(this).data('section-id')+"&roi="+getUrlVars()['roi'];
		sessionStorage.setItem( 'ajaxcall', url );
		sessionStorage.setItem( 'currentroi', getUrlVars()['roi'] );
		
		loadURL(url, $('#content'));
	});
	
	$('li#sections ul li a').click(function(e) {		
		e.preventDefault();

		if( $(this).data('section-id') == 'summary' )
		{
			url = "ajax/summary.php?id="+$(this).data('section-id')+"&roi="+getUrlVars()['roi'];
	...
	$('li#pdf a').click(function(e) {
		e.preventDefault();
		
		url = "ajax/pdf.php?id="+$(this).data('section-id')+"&roi="+getUrlVars()['roi'];
		sessionStorage.setItem( 'ajaxcall', url );
		sessionStorage.setItem( 'currentroi', getUrlVars()['roi'] );
...
function nav_page_height() {
	...
	if (setHeight > windowHeight) {// if content height exceedes actual window height and menuHeight
		$.left_panel.css('min-height', setHeight + $.navbar_height + 'px');
...
function runAllForms() {
	 ...
	 */
	if ($.fn.slider) {
		$('.slider.percent').slider( { 'tooltip':'always', 'formater':function(value){ return numberWithCommas(value) + ' %' } } );
		$('.slider.currency').slider( { 'tooltip':'always', 'formater':function(value){ return '$ ' + numberWithCommas(value) } } );
		$('.slider.noformat').slider( { 'tooltip':'always' } );
		$('.slider.pods').slider( { 'tooltip':'show', 'formater':function(value){ return 'Conservative Factor: ' + numberWithCommas(value) + ' %' } } );
		$('.slider.notooltip').slider( { 'tooltip':'hide' } );
		$('.slider.months').slider( { 'tooltip':'always', 'formater':function(value){ return value + ' months' } } );
		$('.pod-con-slider').slider( { 'tooltip':'hide' } );
	}
...
if($.navAsAjax)
{
    ...
    $(document).on('click', 'nav a[href!="#"]', function(e) {
		...
		if( $this.data( 'link-type' ) != 'absolute' )
		{
			e.preventDefault();
