...
;(function($, window, document, undefined) {
    ...
    RoiShop.prototype = {
		...
		input: function() {
			...
			if(this.options.el_formula) this.$input.attr('data-cell', this.options.el_formula);
		...
		tabs: function(){
			...
			if(tabs){
				$.each(tabs, function(count, tab){
					var $listitem = $(sprintf('<li class="%s"/>', that.options.el_value ? ( that.options.el_value == count ? 'active' : '' ) : ( count == 0 ? 'active' : '' ))),
						$ahref = $(sprintf('<a data-toggle="tab" href="#tab-%s">%s</a>', tab.el_id, tab.el_text));
		...
		graph: function(){
			...
			var $graph = $('<div/>').addClass('graph-holder');
			
			this.el.append($graph);
			this.options.highchart.chart = {};
			this.options.highchart.chart.renderTo = $graph[0];

			var chart = new Highcharts.Chart(this.options.highchart);
		},
		...
		tablecell: function(){
			...
			var hyperlink = false;
			if (this.options.el_text && this.options.el_text.indexOf('href') > -1){
				hyperlink = true;
			}
		...
		slider: function(){
			...
			$cell = this.options.el_letter + this.options.el_name
		...
		select: function(){
			...
			$.proxy(this.onSelectChange, this)			
		},
		...
		onSelectChange: function(){
			...
			var action = selectedOption.ch_action;
			if(action) this.$select.roishopActions({el_action: action});
		},
		...
		tooltip: function($el){
			this.$tooltip = $([
				sprintf('<span class="input-group-addon input right helper %s">', this.options.el_enabled ? 'output' : ''),
		...
		initializeSlider: function($el){
			...
			var onSlideMove = function( $value ){
				...
				$parent.roishop('updateLabel', 'New Label');
