...
sheet.prototype = {
    init: function(){
        ...
        // Register cells that need to be expanded
        if(sheet.cells['RP1']){
            var count = sheet.cells['RP1'].value ? sheet.cells['RP1'].value : 1;
            $.each(expand, function(){
                ...
                for(var i = start; i < parseInt(count) + parseInt(start); i++){
                    var $cell = {};

                    $cell.address = address + i;
                    $cell.format = format;
                    if(label) $cell.label = label.replace(/{{count}}/g, i);
                    if(formula) $cell.formula = formula.replace(/{{count}}/g, i);

                    $cell = new cell(sheet, $cell);
                    sheet.registerCell($cell);
                }
            })
        }
...
cell.prototype = {
    ...
    setVisibility: function(){
        ...
        var elementVisibility = function(cell){
            if(cell.rules) {
                try{
            ...
            return rules && rules.visibility ? rules.visibility : null;
        }

        var getChild = function(parent, visible){
            visibility = elementVisibility(parent);

            if(visibility){
                var el_show = [];
                var el_hide = [];

                $.each(visibility, function(){
                    var operator = this.operator ? this.operator : '==',
                        evaluator = ( parent.value || 0 ) + operator + this.value;
                    ...
                    if(show) {
                        $.each(this.show, function(){
                            var myVisibility = visible && evaluation ? true : false;

                            if(myVisibility) {
                                if(el_show.indexOf(this.trim()) == -1){
                                    el_show.push(this.trim());
                                }
                            } else {
                                if(el_hide.indexOf(this.trim()) == -1){
                                    el_hide.push(this.trim());
                                }                                   
                            }

                            if(roishop.current.options.elements[this.trim()]){
                                var address = roishop.current.options.elements[this.trim()].address;
                                if(address){
                                    getChild(parent.sheet.cells[address], myVisibility);
                                }
                            }                       
                        });
                    } 
                ...
                var elements_to_hide = el_hide.filter((el) => !el_show.includes(el)),
                    elements_to_show = el_show;

                $.each(elements_to_show, function(){
                    if(cell.sheet.elementsToShow.indexOf(this+'') == -1){
                        cell.sheet.elementsToShow.push(this+'');
                    }
                });

                $.each(elements_to_hide, function(){
                    if(cell.sheet.elementsToHide.indexOf(this+'') == -1){
                        cell.sheet.elementsToHide.push(this+'');
                    }
                });
            }
        }
