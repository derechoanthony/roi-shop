;(function (global, factory) {
    ...
    var jexcel = (function(el, options) {
        ...
        obj.createTable = function() {
            ...
            obj.searchInput.setAttribute('placeholder', obj.options.text.search);
            ...
            if(obj.options.showRowNumbering){
                var tempCol = document.createElement('col');
                tempCol.setAttribute('width', obj.options.showRowNumbering ? '50px' : '0');
            ...
            if(obj.options.tableWidth) obj.table.setAttribute('style', 'width: ' + obj.options.tableWidth);
        ...
        obj.createRow = function(j, data) {
            ...
            if(obj.options.showRowNumbering){
                ...
                td.innerHTML = obj.options.rows[j] && obj.options.rows[j].title ? obj.options.rows[j].title : parseInt(j + 1);
                td.setAttribute('data-y', j);
                td.className = 'jexcel_row';
                td.classList.add('numbering');
        ...
        obj.createCell = function(i, j, value) {
            ...
            // Text align
            var colAlign = obj.options.columns[i].align ? $.type(obj.options.columns[i].align) == 'object' && obj.options.columns[i].align.cells ? obj.options.columns[i].align.cells : obj.options.columns[i].align : 'center';
        ...
        obj.createCellHeader = function(colNumber) {
            ...
            var colAlign = obj.options.columns[colNumber].align ? $.type(obj.options.columns[colNumber].align) == 'object' && obj.options.columns[colNumber].align.header ? obj.options.columns[colNumber].align.header : obj.options.columns[colNumber].align : obj.options.defaultColAlign;
            ...
            if(obj.options.columns[colNumber]['calc-id']){
                obj.headers[colNumber].setAttribute('calc-id', obj.options.columns[colNumber]['calc-id']);
            }
        ...
        obj.closeEditor = function(cell, save) {
            ...
            if (save == true) {
                ...
                if (obj.options.data[y][x] == value) {
                    ...
                    obj.setValue(cell, value);
                    if(roishop.current && cell.hasAttribute('calc-id')){
                        var unformatted = numeral().unformat(value); 
                        roishop.current.setValue(cell.getAttribute('calc-id'), unformatted);
                    }
                }
            } else {
                if (obj.options.columns[x].editor) {
         ...
         * @return void
         */
        obj.setAttribute = function(cell, attribute){
            if (typeof(cell) == 'string') {
                ...
                $.each(attribute, function(a, id){
                    if(a == 'readonly' && id == 'true'){
                        obj.records[y][x].classList.add('readonly');
                        return;
                    }
                    obj.records[y][x].setAttribute(a, id);
                });
            }             
        }
         ...
         * @return void
         */ 
        obj.setAttributes = function(){
            if(! obj.options.attributes) return;

            $.each(obj.options.attributes, function(cell, attribute){
                obj.setAttribute(cell, attribute);
            });
        }
         ...
         * Go to page
         */
        obj.page = function(pageNumber) {console.log(pageNumber);
