<?php
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST'){
		switch($_POST['action']){
			...
			case 'updateEntryChoice':
				$apiActions->updateEntryChoice();
			break;
			...
			case 'updateChoicePosition':
				$apiActions->updateChoicePosition();
			break;
		...
		switch($_GET['action']){
			...
			case 'design':
				...
				$design['entries'] = $apiActions->getStructureEntries();
				$design['calculations'] = $apiActions->getStructureCalculations();
	...
	class ApiActions {
		...
		public function deleteChoice(){
			...
			$sql = "DELETE FROM entry_choices WHERE id = :choice;";

			$stmt = $this->_db->prepare($sql);
			$stmt->bindParam(':choice', $_POST['choice'], PDO::PARAM_INT);
			$stmt->execute();				
		}
		...
		public function addNewChoice(){
			$sql = "INSERT INTO entry_choices (entryid, show_map, value, dropdown_value, position)
					VALUES(:entry, :show, :text, :value, '1');";
		...
		public function getStructureCalculations(){
			$sql = "SELECT * FROM cells
					WHERE version = :structure";
		...
		public function getStructureEntries(){
			$sql = "SELECT * FROM entry_fields
					WHERE roiID = :structure";
		...
		public function getEntriesBySection($section){
			...
			foreach($entries as $entry){
				$entries[$i]['choices'] = $this->getEntryChoices($entry['ID']);
				$i++;
			}
		...
		public function getEntryChoices($entry){
			$sql = "SELECT * FROM entry_choices 
					WHERE entryid = :entry
					ORDER BY position ASC;";
		...
		public function updateEntryChoice() {

			$sql = "UPDATE entry_choices SET show_map = :show, value = :text, dropdown_value = :value
					WHERE id = :id";
		...
		public function updateChoicePosition(){
			$sql = "UPDATE entry_choices SET position = :position
					WHERE id = :choice";
