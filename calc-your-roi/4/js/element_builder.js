...
function getCellValueArray(el){
	...
	function __getElementType(value){
		...
		if(visibility != "none") {		
			...
			switch(value.type){
				...
				case 'input':
					element_build += __createInput(value);
					break;
				...
				case 'table':
					element_build += __createTable(value);
					break;
				...
				case 'quotes':
					element_build += __createQuotes(value);
					break;
			}
		...
		$.each(specs.series, function( index, value ){
			...
			if(value.data && value.data.length !== 0){
				$.each(value.data, function( index, value ){
					...
					graph += '<input class="graph-formula" data-formula="' + value.equation + '"' + ( value.sliced == "true" ? ' data-sliced' : '' ) + ( value.name ? ' data-series-name="' + value.name + '"' : '' ) + '/>';
				});
			}
			...
			for( var i=0; i<repeat; i++ ){
				...
				if(value.cells) {
					...
					$.each(value.cells, function( index, value ){
						...
						table_cell += '>' + ( value.type == 'input' ? '' : value.content ) + '</span>' + ( value.append ? value.append : '' ) + '</td>';
	...
	function __createInput(specs) {
		...
		*/
		...
		input_attributes.cell_reference = 'data-cell-reference="' + ( specs.letter ? specs.letter : 'A' ) + specs.id + '"';
		input_attributes.cell = 'data-cell="' + ( specs.letter ? specs.letter : 'A' ) + specs.id + '"';
	...
	function __createSlider(specs) {
		...
		*/
		...
		slider_attributes.cell = 'data-cell="A' + specs.id + '"';
		slider_attributes.format = 'data-format="' + specs.format + '"';
	...
	function __createDropdown(specs){
		...
		*/
		...
		$.each(specs.options, function( index, value ){
			dropdown += '<option value="' + value.value + '"' + ( value.showmap ? ' data-show-map="' + value.showmap + '"' : '' ) + '>' + value.text + '</option>';
		});
	...
	function __createQuotes(specs) {
		
		var quotes = '<div class="quotes">';
		
		$.each(specs.testimonials, function(index, value){
			quotes += '<div id="blockquote" class="row" style="min-height: 220px; margin: 0;">'
			
			quotes += '<blockquote ' + ( value.author == 'twitter' ? 'class="twitter-tweet" lang="en" data-conversion="none"' : '' ) + '>';
			quotes += '<p>' + value.testimonial + '</p>' + ( value.author && value.author != 'twitter' ? '<p> — ' + value.author + '</p>' : '' )
				...
				if(value.sections && value.sections.length !== 0){
					...
					$.each(value.sections, function( index, value ){
						leftSidebar += '<li class="section-navigator" data-section-id="' + value.id + '">\
											<a ' + ( value.action ? 'id="' + value.action + '" ' : 'href="' + value.href + '"' ) + '>' + value.label + '</a>\
										</li>';
					});
