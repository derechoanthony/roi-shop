...
class db_interaction {	
	...
	private function mailConfirmation(){
		...
		$sql = "SELECT * FROM roi_users
			...
			if( ! in_array ( 'jacobachorn@yahoo.com', $optedout ) ){
				$maillist[] = array( 'jacobachorn@yahoo.com', 'Jacob Achorn', 'roi' );
			}
		...
		if( $maillist ){
			...
			for($i=0;$i<count( $maillist );$i++){
				...
				$bodyHtml = file_get_contents(realpath($_SERVER['DOCUMENT_ROOT'])."/enterprise/assets/email/viewed.html");
				$bodyHtml = str_replace('%name%', $ver, $bodyHtml);
				$bodyHtml = str_replace('%creator%', ($maillist[$i][2]!='rep'?$data['username']:''), $bodyHtml);
				$bodyHtml = str_replace('%ipaddress%', $_SERVER["HTTP_X_FORWARDED_FOR"], $bodyHtml);
				$bodyHtml = str_replace('%link%', ($maillist[$i][2]!='manager'?$_GET['roi']:$_GET['roi'].'&v='.$_GET['v']), $bodyHtml);
				$bodyHtml = str_replace('%views%', $hits['visits'], $bodyHtml);
				$bodyHtml = str_replace('%unique%', $hits['unique_ip'], $bodyHtml);					
					
				foreach ($ipInfo as $k => $v) {
					switch($k) {
						case 'country':
							$bodyHtml = str_replace('%country%', $v, $bodyHtml);
							break;
						case 'stateprov':
							$bodyHtml = str_replace('%state%', $v, $bodyHtml);
							break;	
						case 'city':
							$bodyHtml = str_replace('%city%', $v, $bodyHtml);
							break;
					}
				}				
