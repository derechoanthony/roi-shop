...
;(function (global, factory) {
	...
	var roishop = (function(el, options) {
		...
		var verification = calc.options.verification;
		var pdfs = calc.options.pdfs;
		var navigation = calc.options.navigation;
		var roi = calc.options.specs.roi_id;
		var currency = calc.options.specs.currency;
		var verificationCode = calc.options.specs.verification_code;
		var visits = calc.options.specs.visits;
		var unique = calc.options.specs.unique_ip;
		var title = calc.options.specs.roi_title;
		...
		// Extend Structure Options
		if(version.structure.options) {
			var opts = $.parseJSON(version.structure.options);
				version.structure = $.extend(true, version.structure, opts);
		}
		
		// Extend Company Options
		if(version.structure.company.options) {
			var opts = $.parseJSON(version.structure.company.options);
				version.structure.company = $.extend(true, version.structure.company, opts);
		}
		...
		calc.sharedCells = function(){
			...
			cells.push({
				...
				value: window.location.href.match(/^.*\//) + '?roi=' + roi + '&v=' + verificationCode
			});
			...
			try {
				cells.push({
					address: 'CREATE1',
					label: 'Date Created',
					value: calc.options.specs.dt.split(' ')[0]
				});
			} catch(e){}
			...
			try {
				cells.push({
					address: 'CREATEBY1',
					label: 'Created By',
					value: calc.options.specs.createdBy
				});
			} catch(e){}
			...
			try {
				cells.push({
					address: 'CREATEFOR1',
					label: 'Created For',
					value: calc.options.specs.roi_title
				});
			} catch(e){}
			...
			var payback = 'MIN( ( ( ABS( TC1 ) / ( ( GT1 - TC1 ) / ( ( ' + term + ' * 12 - ROUNDUP( IMP1 / 12, 0 ) * 12 ) / 12 + ( ROUNDUP( IMP1 / 12, 0 ) * 12 - IMP1 ) / 12 ) ) ) * 12 + IMP1 ), ' + term + ' * 12 )';
		...
		calc.mergeValues = function(){
			...
			if(calc.options.oldValues){
				$.each(calc.options.oldValues, function(){
					...
					if(!included){
						if(this.entryid && $.isNumeric(this.entryid.slice(0,1))){
				...
				$.each(parsedValues, function(){
					if('value' in this) {
						...
						if(this.customformula){
							this.forcedValue = this.customformula;
						}

						if(this.formattedValue && (this.formattedValue+'').includes('%')){
							this.value = ( this.formattedValue.replace('%', '') / 100 );
						}
					}
				});
			}

			if(calc.builder.cells && calc.builder.cells.length){
				$.each(calc.builder.cells, function(){
					...
					if(this.address == "VISIT1"){
						this.value = visits;
					}

					if(this.address == "UNIQUE1"){
						this.value = unique;
					}
		...
		calc.sections = function(){
			...
			$.each(sections, function(i, section){
				...
				var sectionId = section.section_id || section.ID,
					...
					contentSections = section.sections ? section.sections : ( section.formula || ( section.statistics > 0 && section.snapshot !== false ) ? [{
				...
				if(entryBySection){
					...
					$.each(entryBySection, function(i, entry){
						...
						calc.options.elements['#' + entry.ID] = entry;
		...
		calc.sectionSidebar = function(section, $parent){
			...
			if(section.sidebar && section.sidebar.build){
				...
				if(section.formula){
					if(section.statistics > 0 && section.snapshot !== false){
				...
				} else if(section.statistics > 0 && section.snapshot !== false){
					$parent.append($statistics);
				}
				...
				if(section.formula || ( section.statistics > 0 && section.snapshot !== false ) ){
							...
							if(graph.length){
								$.each(graph, function(){
									if(this && this.graph_build){
										...
										var ajax = function(url, data, yes, no){
													...
													setTimeout(function(){
														rsAjax({
															data: {
																...
																roi_id: getQueryVariable('roi'),
																reportId: reportId,
																roiPath: window.location.href + '&v=' + verificationCode,
																graphs: graphs
															},
															success: function(returned){
																waitingDialog.hide();
																$('<a href="/webapps/assets/customwb/10016/pdf/' + title + '.pdf" download>')[0].click();
															},
										...
										// 		width: graph.width || 700
										// 	},
										// 	success: function(img){console.log(img);
										// 		rsAjax({
										// 			type: "POST",
										// 			data: {
										// 				action: "storeChartImage",
										// 				image: img
										// 			}
										// 		})
	...
	roishop.storeValues = function(){
		var calc = roishop.current,
			cells = calc.sheet.cells,
			values = [];
	...
	roishop.calculationBreakdown = function(cell){
			...
			for(var dependent in dependencies){
				...
				if(has_dependencies){
					...
					for (dependent in dependentDependencies){
						...
						if (formula_txt) formula_txt = formula_txt.replaceAll(dependent, dependent_opts.label);
					}
		...
		for(var dependent in dependencies){
			if(formula_text) formula_text = formula_text.replaceAll(dependent, dependencies[dependent].label ? dependencies[dependent].label : dependencies[dependent].address )
		}
	...
	// console.log('*[retrieveCalcYourRoi] : ',rsAjax({
	// 	data: {
	// 		action: 'retrieveCalcYourRoi',
	// 		roi_id: getQueryVariable('roi'),
	// 		verifiction: getQueryVariable('v')
