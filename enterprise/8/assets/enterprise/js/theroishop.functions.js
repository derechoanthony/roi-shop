;(function($, window, document, undefined) {
	...
	var topNavigation = function(RSCalc){
		...
		if(options.verification > 1){
			...
			allowedActions.push('<li><a class="addContributor">Add Allowed User</a></li>');
			allowedActions.push('<li><a class="viewAllowedUsers">View Allowed Users</a></li>');
		}
	...
	var Wizard = function(options, RSCalc){
		var wizard = this;
		...
		wizard.options = $.extend(true, {}, defaults, options)
		if(options.options) wizard.options = $.extend(true, wizard.options, JSON.parse(options.options));

		wizard.$container = $('<div/>');
		wizard.$nav = wizard.$container.children('ul');
		wizard.$steps = $('li > a', wizard.$nav);
		wizard.$steps_container = wizard.$container.children('div');
		wizard.$pages = wizard.$steps_container.children('div');

		options.$container.append(wizard.$container);

		wizard.init();
	}

	Wizard.fx = Wizard.prototype;

	Wizard.fx.init = function(){
		var wizard = this;
	...
	Wizard.fx.setElements = function(){
		var wizard = this;

		wizard.$container.addClass('wizard-main wizard-theme-' + wizard.options.theme);
		wizard.$nav.addClass('nav nav-tabs step-anchor').children('li').addClass('nav-item').children('a').addClass('nav-link');

		if (wizard.options.anchorSettings.enableAllAnchors !== false && wizard.options.anchorSettings.anchorClickable !== false) {
			wizard.$steps.parent('li').addClass('clickable');
		}		
	}

	Wizard.fx.setToolbar = function(){
		console.log(this);
	}

	Wizard.fx.setEvents = function(){
		console.log(this);
	}
	...
	Checkbox.fx.build = function(){
		...
		if(options.choices){
			$.each(options.choices, function(i, choice){
				checkbox.$checkbox.append(sprintf('<div class="checkbox i-checks"><label><input type="checkbox" value="%s"%s><i></i> %s</label></div>', choice.value, choice.selected == '1' ? ' checked="checked"' : '', choice.text));
			});
		};
	...
	Section.fx.body = function(){
		...
		if(options.Caption){
			...
			if(section.rsCalculator.options.testimonials > 0){
				var $testimonials = [
					'<div class="quotes">',
				];
	...
	RSCalc.includeButtons = function(){
		$('.btn-include').off('click').on('click', function(){
	...
	RSCalc.assignValues = function(){
		...
		var currentValues = RSCalc.options.values[0];
	...
	RSCalc.intializeSlider = function(slider){
		...
		$slider.noUiSlider({
			start: 0,
			connect: 'lower',
			step: $slider.hasClass('implementation_period') ? 1 : ( $slider.hasClass('conservative_slider') ? 5 : 1 ),
			range: {
				min: 0,
