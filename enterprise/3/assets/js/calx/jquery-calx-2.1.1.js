...
financial: {
    ...
    COUNTIF : function(range, criteria) {
		var matches = 0, i;
        for (i in range) {
			if (range[i].toString().match(new RegExp(criteria))) {
                matches++;
            }
        }
        return matches;
...
cell.fx.checkCircularReference = function(address){
    ...
    return isCircular;
};cell.fx.getAllDependents = function(address, inputArray){
	...
	for(a in this.dependant){
		...
		if($.inArray(a, inputArray) == -1 ) {
			inputArray.push(a);

			if(!$.isEmptyObject(this.dependant[a].dependant)){
				this.dependant[a].getAllDependents(address, inputArray);
			}			
		}
	}
	
	return inputArray;
};cell.fx.calculateAllDependents = function(address){
	...
	for(a in this.dependant){
		...
		this.dependant[a].calculate(false, true);
		if(!$.isEmptyObject(this.dependant[a].dependant)){
			this.dependant[a].calculateAllDependents(address);
		}
	}
};(false, true);cell.fx.getAllDependencies = function(address){
	//console.log('cell[#'+this.sheet.elementId+'!'+this.address+'] : checking circular reference');
	var a, allDependencies = '', inputArray = [];
	...
	if(false !== this.formula){
		for(a in this.dependencies){
			...
			if($.isEmptyObject(this.dependencies[a].dependencies) && $.inArray(inputReference,inputArray)==-1){
				
				inputArray.push(inputReference);
				var inputLabel = $('#'+inputReference).closest('.form-group').find('label').html();
				var inputValue = $('#'+inputReference).closest('.form-group').find('input').val() || '0';
				
				allDependencies += '<div class="form-group" style="margin-bottom: 0px;">\
										<label class="control-label col-lg-9 col-md-5 col-sm-5" for="disc_1">' 
											+ inputLabel +
										'</label>\
										<label class="pull-right control-label col-lg-3 col-md-5 col-sm-5"\
											<b>' + inputValue + '</b>\
										</label>\
									</div>';
			}
		...
		for(a in this.dependencies){
			if(!$.isEmptyObject(this.dependencies[a].dependencies)){
				allDependencies += this.dependencies[a].getAllDependencies(address);
			}
		}
	}

	return allDependencies;
};cell.fx.buildInputPanels = function(address){
	...
	if(false !== this.formula) {
		for(a in this.dependencies){
			...
			var cellNumber = a.replace(/[A-Z]/g,'');
			var cellType = a.replace(cellNumber,'');
					
			if(cellType != 'U') {
						
				// Get the cell's label
				var cellLabel = $('#'+a).closest('.form-group').find('label').html();
				
				// Get the current cell's value
				var inputValue = $('#'+a).closest('.form-group').find('input').val() || '0';
				
				// Get input add on if there is one
				var appendValue = $('#'+a).closest('.form-group').find('.input-group-addon').find('select').find(':selected').text() || $('#'+cellNumber).closest('.form-group').find('.input-group-addon').html() || '';
						
				var dropdownTag = false;
				if(!$.isEmptyObject(this.dependencies[a].dependencies)){
					dropdownTag = true;
				}
				
				// Build the cell panel
				panel += 	'<div class="panel-heading">\
								<h5 class="panel-title collapsed" href="#panel' + a + '" data-toggle="collapse" aria-expanded="false">\
									<'+ ( dropdownTag ? 'a' : 'p' ) + ' style="margin-bottom: 0;">' + ( dropdownTag ? '<i class="fa fa-plus-circle" style="color: blue;"></i> ' : '' ) + cellLabel + '<span class="pull-right">' + inputValue + ' ' + appendValue + '</span></' + ( dropdownTag ? 'a' : 'p' ) + '>\
								</h5>\
							</div>';
							
				if(!$.isEmptyObject(this.dependencies[a].dependencies)){
					panel += '<div id="panel' + a + '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px; margin: 0 0 0 15px; padding: 10px; background-color: #eee; border-left: 3px solid blue;">';
					
					cellFormula = this.dependencies[a].formula;
					for(b in this.dependencies[a].dependencies){
						var inputReference = b.replace(/[A-Z]/g,'');
						var inputCellType = b.replace(inputReference,'');
						var inputLabel = $('#'+inputReference).closest('.form-group').find('label').html() + ( inputCellType == 'U' ? ' (unit factor)' : '' );
						cellFormula = cellFormula.replace(b,inputLabel);
					}
					
					cellFormula = cellFormula.replace("/100","").replace("/ 100","");

					var panelFormula =	'<div class="widget style1" style="margin: 15px; border: 1px solid #999;"> <strong>Equation: ' + cellFormula + ' </strong></div>';
					var panelInputs = 	this.dependencies[a].buildInputPanels(address);
					panel += panelInputs + panelFormula;
					panel +=	'</div>';				
				}
			}			
		}
