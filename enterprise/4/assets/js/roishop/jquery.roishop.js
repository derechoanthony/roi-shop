;(function($, window, document, undefined) {
	...
	RoiShopCalculator.prototype.resetTemplate = function() {
		$.post("/enterprise/4/assets/ajax/calculator.post.php",{
					...
					switch(i){
						case 'moveTableRow':
							var element_to_move = actions[i][0],
								move_to_table = actions[i][1],
								row_data = $(sprintf('[element-id="%s"]', element_to_move)).data('element.options'),
								table = $(sprintf('[element-id="%s"]', element_to_move)).closest('.bootstrap-table').parent().data('element.options');
			...
			var Mode = {
				edit: function(td){
					...
					var $cell = $(td),
						...
						value = options.el_value ? options.el_value : options.el_text,
	...
	RoiShopCalculator.prototype.toggle = function($container, options){
		...
		var toggleToggle = function(){
			...
			if (choices[isChecked].ch_show){
				var show_elements = choices[isChecked].ch_show.split(',');
			}
			
			if (choices[isChecked].ch_hide){
